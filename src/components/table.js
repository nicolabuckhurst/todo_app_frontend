        ...
        for(let i=0; i<this.props.tasks.length;i++){
            ...
            if(this.props.tasks[i].completed == false){
                tableRowsToRender.unshift(
                    <TableRow key={this.props.tasks[i].id} task={this.props.tasks[i]} toggleCompleteStatus={this.props.toggleCompleteStatus}  /> //react needs this key to identify when an aray element changes
                )
            } else {
