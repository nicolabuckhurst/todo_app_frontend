        ...
        for(let i=0; i<this.props.tasks.length;i++){
            ...
            if(this.props.tasks[i].completed == false){
                tableRowsToRender.unshift(
                    <TableRow key={this.props.tasks[i].id.toString() + this.props.tasks[i].completed.toString() } task={this.props.tasks[i]} toggleCompleteStatus={this.props.toggleCompleteStatus} toggleShowStatus={this.props.toggleShowStatus} deleteTask={this.props.deleteTask}  /> //react needs this key to identify when an aray element changes
                )
            } else {
            //if task is complete add to the end of the array..completed tasks will appear in order they were added most recent last
                tableRowsToRender.push(
                    <TableRow key={this.props.tasks[i].id + this.props.tasks[i].completed.toString()} task={this.props.tasks[i]} toggleCompleteStatus={this.props.toggleCompleteStatus} toggleShowStatus={this.props.toggleShowStatus} deleteTask={this.props.deleteTask}  /> //react needs this key to identify when an aray element changes
                )
            }
        }
